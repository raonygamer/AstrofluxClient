package {
	...
	public class Login extends Sprite {
		...
		private function onConnectSimple(e:TouchEvent = null) : void {
			var _local2:String = Util.trimUsername(emailInput.text);
			var _local3:String = Util.trimUsername(passwordInput.text);
			emailInput.error = "";
			passwordInput.error = "";
			hideLogin();
			updateStatus(Localize.t("Connecting to Server"));
			mySharedObject.data.email = _local2;
			mySharedObject.flush();
			PlayerIO.quickConnect.simpleConnect(Starling.current.nativeStage,gameId,_local2,_local3,handleConnect,handleLoginError);
		}
		...
		private function handleConnectFacebook(client:Client, token:String) : void {
			...
			FB.api("/v2.2/me?fields=first_name,friends,locale&",function(param1:Object):void {
				...
				if(param1.friends && param1.friends.data) {
					_local2 = param1.friends.data;
				}
				_local3 = 0;
				while(_local3 < _local2.length) {
					joinData.friends.push("fb" + _local2[_local3].id);
					_local3++;
				}
		...
		private function addParticleFlow() : void {
			...
			var _local3:Texture = assets.getTexture("star");
			var _local1:Array = [15907921,6210749,14305875];
			_local4 = 0;
			while(_local4 < 100) {
				_local2 = new Image(_local3);
				_local2.blendMode = "add";
				_local2.x = 0;
				_local2.y = 0;
				_local2.alpha = 0.4;
				_local2.color = _local1[Math.floor(Math.random() * _local1.length)];
				addParticleTween(_local2);
				effectContainer.x = stage.stageWidth / 2;
				effectContainer.y = logoContainer.y + logoContainer.height / 2;
				effectContainer.addChildAt(_local2,0);
