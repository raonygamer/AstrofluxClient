package core.hud.components {
	...
	public class CrewDetails extends Sprite {
		...
		public function CrewDetails(g:Game, crewMember:CrewMember, requestReloadCallback:Function = null, showTraining:Boolean = true, mode:int = 0) {
			...
			super();
			if(crewMember == null) {
				_local9 = new Text(15,72,true);
				_local9.text = Localize.t("You can unlock another crew slot in the ship overview.");
				_local9.size = 14;
				_local9.width = 310;
				addChild(_local9);
				return;
			...
			img.x = 4 * 60;
			addChild(img);
			var _local8:Text = new Text(0,0);
			_local8.text = crewMember.name;
			_local8.color = 16623682;
			_local8.size = 26;
			var _local7:Text = new Text(0,35);
			_local7.text = CrewMember.getRank(crewMember.rank);
			_local7.color = 16689475;
			addChild(_local7);
			var _local11:TextBitmap = new TextBitmap(0,_local7.y + 15,crewMember.missions + " " + Localize.t("missions"));
			_local11.format.color = 0x686868;
			addChild(_local11);
			addChild(_local8);
			addChild(img);
			addSkills();
		...
		private function addCurrentStatus() : void {
			...
			if(crewMember.isDeployed) {
				if(crewMember.isWaitingForPickup) {
					_local1.text = Localize.t("Awaiting pickup");
					_local1.size = 16;
				} else {
					_local1.text = Localize.t("Exploring [location]").replace("[location]",crewMember.getCompactFullLocation());
				}
			} else if(crewMember.isTrainingComplete && crewMember.isTraining) {
				_local1.text = Localize.t("Training complete");
				_local1.size = 16;
				addClaimTraining(_local2);
			} else if(crewMember.isTraining) {
				_local1.size = 16;
				_local1.text = Localize.t("Training...");
			} else if(crewMember.isInjured) {
				_local1.size = 16;
				_local1.color = Style.COLOR_INJURED;
				_local1.text = Localize.t("Injured...");
			}
			_local2.addChild(_local1);
			_local2.y = 455;
			statusTween = TweenMax.to(_local1,1,{
				"alpha":0.5,
				"yoyo":true,
				"repeat":-1
			});
		...
		private function addSpecialSkills() : void {
			...
			var _local2:Text = new Text(0,0);
			_local2.text = Localize.t("Special skills") + ":";
			_local2.color = 16689475;
			specialSkillsHolder.addChild(_local2);
			nextY = 25;
			_local3 = 0;
			while(_local3 < 9) {
				_local1 = addOrUpdateSpecialSkill(_local3);
				if(_local3 == 5) {
					nextY -= 125;
				}
				_local1.y = nextY;
		...
		private function isSpecialVisible(i:int) : Boolean {
			var _local2:String = Area.SPECIALTYPE[i];
			var _local4:int = crewMember.getSkillValueByName("Survival");
			var _local5:int = crewMember.getSkillValueByName("Diplomacy");
			var _local3:int = crewMember.getSkillValueByName("Combat");
			if(_local2 == "Cold" && _local4 >= 20) {
				return true;
			}
			if(_local2 == "Heat" && crewMember.hasUnlockedSpecialSkill("Cold")) {
				return true;
			}
			if(_local2 == "Radiation" && crewMember.hasUnlockedSpecialSkill("Heat")) {
				return true;
			}
			if(_local2 == "First Contact" && _local5 >= 20) {
				return true;
			}
			if(_local2 == "Trade" && crewMember.hasUnlockedSpecialSkill("First Contact")) {
				return true;
			}
			if(_local2 == "Collaboration" && crewMember.hasUnlockedSpecialSkill("Trade")) {
				return true;
			}
			if(_local2 == "Kinetic Weapons" && _local3 >= 20) {
				return true;
			}
			if(_local2 == "Energy Weapons" && crewMember.hasUnlockedSpecialSkill("Kinetic Weapons")) {
				return true;
			}
			if(_local2 == "Bio Weapons" && crewMember.hasUnlockedSpecialSkill("Energy Weapons")) {
				return true;
			}
			return false;
		}
		...
		private function raiseSpecialSkill(type:int) : void {
			...
			g.rpc("raiseCrewSpecialty",function(param1:Message):void {
				var _local3:int = param1.getInt(0);
				var _local2:Number = param1.getNumber(1);
				crewMember.specials[_local3] = _local2;
				addOrUpdateSpecialSkill(_local3,_local2);
				if(_local2 >= 1) {
					playSkillUnlockAnimation(_local3,"aquired!");
				}
				updateSkillPoints();
			},crewMember.key,type);
		}
		...
		private function onAcceptTraining(e:Event) : void {
			...
			g.rpc("startTraining",function(param1:Message):void {
				var _local4:Boolean = param1.getBoolean(0);
				if(!_local4) {
					g.showErrorDialog(param1.getString(1),true);
					return;
				}
				var _local3:int = param1.getInt(1);
				var _local2:Number = param1.getNumber(2);
				var _local5:Number = param1.getNumber(3);
				crewMember.trainingType = _local3;
				crewMember.trainingEnd = _local5;
				requestReload();
			},crewMember.key,1);
		}
		...
		private function getSpecialIcon(i:int, gray:Boolean = false) : Image {
			...
			if(gray) {
				_local4 = new Image(_local3.getTextureGUIByTextureName(IMAGES_SPECIALS[i] + "_inactive"));
			} else {
				_local4 = new Image(_local3.getTextureGUIByTextureName(IMAGES_SPECIALS[i]));
			}
			return _local4;
		}
