package core.hud.components {
	...
	public class ScreenTextField extends TextField {
			...
			if(currentParagraph < _textArray[currentPage].length - 1) {
				...
				pauseTimer.addEventListener("timerComplete",(function():* {
					...
					return onPauseComplete = function(param1:flash.events.Event):void {
						...
						if(_textArray != null) {
							_local3 = _textArray[currentPage];
							if(_local3 == null) {
								return;
							}
							_local2 = _local3[currentParagraph];
							revealParagraph(_local2,true);
						}
		...
		public function getFullWidth(text:String, theSize:int = 0) : Number {
			...
			if(theSize > 0) {
				_local4.size = theSize;
			} else {
				_local4.size = format.size;
			}
			_local4.text = text;
			var _local3:Number = _local4.width;
			_local4.clean();
			_local4 = null;
			return _local3;
		}
		...
		private function randText(len:int = 6) : String {
			var _local3:int = 0;
			var _local2:Array = new Array(len);
			_local3 = 0;
			while(_local3 < len) {
				_local2[_local3] = charArray[int(Math.random() * charArray.length)];
				_local3++;
			}
			return _local2.join("");
		}
