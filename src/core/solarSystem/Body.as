package core.solarSystem {
	...
	public class Body extends GameObject {
		...
		public function updateIsNear() : void {
			...
			var _local2:Number = g.stage.stageWidth * 1.5;
		...
		public function updateBody(startTime:Number) : void {
			...
			_local4 = 0;
			while(_local4 < spawners.length) {
				spawners[_local4].update();
				spawners[_local4].updatePos(startTime);
				_local4++;
			}
			_local3 = 0;
			while(_local3 < children.length) {
				children[_local3].updateBody(startTime);
				_local3++;
			}
			super.update();
		...
		public function preDraw(obj:Object) : void {
			var _local3:Matrix = null;
			var _local9:Sprite = null;
			var _local5:Sprite = null;
			var _local6:Sprite = null;
			if(obj.type == "hidden" || obj.type == "boss" || obj.type == "sun" || obj.type == "comet") {
				return;
			}
			var _local7:Number = 5 * 60;
			var _local4:Number = 0;
			if(safeZoneRadius > 0) {
				_local3 = new Matrix();
				_local3.createGradientBox(2 * _local7,2 * _local7,0,-_local7,-_local7);
				_local4 = safeZoneRadius / _local7;
				_local9 = new Sprite();
				_local9.graphics.lineStyle(1,0x8888ff,0.2);
				_local9.graphics.beginGradientFill("radial",[0,0,0x8888ff],[0,0,0.2],[0,200,255],_local3);
				_local9.graphics.drawCircle(0,0,_local7);
				...
				this.safeZoneActive.scaleX = _local4;
				this.safeZoneActive.scaleY = _local4;
				this.safeZoneActive.blendMode = "add";
				_local5 = new Sprite();
				_local5.graphics.lineStyle(1,0xff8888,0.2);
				_local5.graphics.beginGradientFill("radial",[0,0,0xff8888],[0,0,0.2],[0,200,255],_local3);
				_local5.graphics.drawCircle(0,0,_local7);
				_local5.graphics.endFill();
				this.safeZoneDisabled = TextureManager.imageFromSprite(_local5,"safeZoneDisabled");
				this.safeZoneDisabled.pivotX = _local5.width / 2;
				this.safeZoneDisabled.pivotY = _local5.height / 2;
				this.safeZoneDisabled.scaleX = _local4;
				this.safeZoneDisabled.scaleY = _local4;
				this.safeZoneDisabled.blendMode = "add";
			}
			if(hostileZoneRadius > 0) {
				_local3 = new Matrix();
				_local3.createGradientBox(2 * _local7,2 * _local7,0,-_local7,-_local7);
				_local4 = hostileZoneRadius / _local7;
				_local6 = new Sprite();
				_local6.graphics.lineStyle(1,0xff8888,0.3);
				_local6.graphics.beginGradientFill("radial",[0,0,0xff8888],[0,0,0.3],[0,200,255],_local3);
				_local6.graphics.drawCircle(0,0,_local7);
				_local6.graphics.endFill();
				this.hostileZone = TextureManager.imageFromSprite(_local6,"hostileZone");
				this.hostileZone.pivotX = _local6.width / 2;
				this.hostileZone.pivotY = _local6.height / 2;
				this.hostileZone.scaleX = _local4;
				this.hostileZone.scaleY = _local4;
				this.hostileZone.blendMode = "add";
			}
			var _local8:Random = new Random(obj.radius * id);
			var _local2:ITextureManager = TextureLocator.getService();
			glowImage = new Image(_local2.getTextureByTextureName("fluff.png","texture_main_NEW.png"));
			...
			if(obj.type == "planet") {
				if(obj.atmosphereColor != null && obj.atmosphereColor != 0) {
					...
					bodyFilter = new GlowFilter(obj.atmosphereColor,0.5,5 * _local4,0.25);
					bodyFilter.cache();
				}
			} else {
		...
		public function setInSafeZone(p:Player) : void {
			...
			var _local2:Point = p.ship.pos;
			var _local3:Number = pos.x - _local2.x;
			var _local4:Number = pos.y - _local2.y;
			if(spawnersCleared && _local3 * _local3 + _local4 * _local4 < safeZoneRadius * safeZoneRadius) {
				p.inSafeZone = true;
				if(p.ship == null) {
					return;
				}
